; After the NES displays a frame of graphics, it stops drawing for a while.
; This period is known as the "vblank", and is a good choice for performing
; graphics updates. Note that this interrupt is non-maskable (NMI).
;
...
.PROC ISR_Vertical_Blank
  ...
  ; ----------------------------
    ...
    sta     _JOY1           ;
    lda     #$00            ;
    sta     _JOY1           ; Controllers for first and second player are now
                            ; latched and will not change

  ; ----------------------------------------------------
  ; Read button A and move sprite right if it's pressed.
  ; ----------------------------------------------------
    lda     _JOY1
    and     #%00000001
    beq     readButtonAEnd
  ...
  ; ----------------------------------------------------
  ; Read button B and move sprite left if it's pressed.
  ; ----------------------------------------------------
    lda     _JOY1
    and     #%00000001
    beq     readButtonBEnd
